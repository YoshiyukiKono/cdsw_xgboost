...
// Switched to use spark.jar.package in spark-defaults.conf
...
val inputPath = "/tmp/iris.csv"
val nativeModelPath = "model"
val pipelineModelPath = "pipeline"

import org.apache.spark.SparkConf
val config = new SparkConf()
config.setExecutorEnv("DMLC_TRACKER_PORT",sys.env("CDSW_HOST_PORT_0"))
//val spark = SparkSession.builder().appName("XGBoost4J-Spark Pipeline Example").getOrCreate()
val spark = SparkSession.builder().config(config).appName("XGBoost4J-Spark Pipeline Example").getOrCreate()
...
//  setInputCols(Array("sepal length", "sepal width", "petal length", "petal width")).setOutputCol("features")
